<template>
    <div>
        <h2 style="font-size: 36px;">一周通行时间排名</h2>
        <br><br>
        <!-- 显示更新时间信息 -->
...
// 定义一个变量来存储图片的 URL
const imageSrc = ref('/images/v4.png'); // 初始图片 URL
...
const recalculateImages = async () => {
    ...
    try {
        const response = await axios.post('http://127.0.0.1:5000/vision/v4/weekrank'); // 替换为您的后端 API 路径
        if (response.data.success) { // 根据后端返回的数据结构判断成功
            ElMessage.success('重新计算成功');
            // 在接收到后端回复后更新当前时间
            updateTime();
            imageSrc.value = '/images/v4.png' + '?' + new Date().getTime();
        } else {
